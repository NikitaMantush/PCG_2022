...
{
    ui->setupUi(this);
    setStyleSheet("background-color: papayawhip;");
    QGridLayout *g = new QGridLayout(centralWidget());
    area = new PlotArea();
    g -> addWidget(area, 0, 0, 10, 4);
    g -> addWidget(ui -> clippingText,  3, 4, 1, 1);
    g -> addWidget(ui -> segments,      4, 4, 1, 1);
    g -> addWidget(ui -> poly,          5, 4, 1, 1);
...
void MainWindow::ProcessSegments()
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {
            //outside
            area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x2, y2), Qt::green});
        }
        else
        {
            qreal A = y2 - y1;
            ...
            while(code1 != 0 || code2 != 0)
            {
                ...
                if (code1 & 1)
                {
                    //leftside xmin
                    qreal y = (-C - A * xmin) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmin, y), Qt::green});
                    x1 = xmin;
                    y1 = y;
                }
                else if (code1 & 2)
                {
                    //rightside xmax
                    qreal y = (-C - A * xmax) / B;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(xmax, y), Qt::green});
                    x1 = xmax;
                    y1 = y;
                ...
                else if (code1 & 4)
                {
                    //bottomside ymin
                    qreal x = (-C - B * ymin) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymin), Qt::green});
                    x1 = x;
                    y1 = ymin;
                }
                else
                {
                    //topside ymax
                    qreal x = (-C - B * ymax) / A;
                    area -> AddLineSegment(LineSegmentData{QPointF(x1, y1), QPointF(x, ymax), Qt::green});
                    x1 = x;
                    y1 = ymax;
                }
                code1 = getCode(x1, y1);
